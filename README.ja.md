...
- 改行区切りの JSON アクセスログを解析します。空行を除く各行には、JSON オブジェクトを
  1 つだけ記述する必要があり、1 つのオブジェクトを複数行に分けることはできません
- 行末の空白は許可します。2 つ目の JSON 値や、オブジェクトの後に空白以外のデータが
  ある場合は、暗黙に無視せずエラーにします
- デフォルトでは以下のキーを抽出します
    - `time`
        - ログ時刻
    - `method`
